package explodingkittens.controller;

import explodingkittens.model.Player;
import explodingkittens.model.Card;
import explodingkittens.model.CardType;
import java.util.List;
import java.util.Scanner;

/**
 * Implementation of CatCardRequestInputHandler that uses console input.
 */
public class ConsoleCatCardRequestInputHandler implements CatCardRequestInputHandler {
    private final Scanner scanner;

    /**
     * Creates a new ConsoleCatCardRequestInputHandler with the given scanner.
     * @param scanner The scanner to use for input
     * @throws IllegalArgumentException if scanner is null
     */
    public ConsoleCatCardRequestInputHandler(Scanner scanner) {
        if (scanner == null) {
            throw new IllegalArgumentException("Scanner cannot be null");
        }
        this.scanner = scanner;
    }

    @Override
    public Player selectTargetPlayer(List<Player> availablePlayers) {
        if (availablePlayers == null || availablePlayers.isEmpty()) {
            throw new IllegalArgumentException("Available players list cannot be null or empty");
        }

        System.out.println("\nAvailable players:");
        for (int i = 0; i < availablePlayers.size(); i++) {
            System.out.println((i + 1) + ". " + availablePlayers.get(i).getName());
        }
        System.out.print("Select a player (1-" + availablePlayers.size() + "): ");
        
        int choice = scanner.nextInt();
        if (choice < 1 || choice > availablePlayers.size()) {
            throw new IllegalArgumentException(
                "Invalid choice. Please enter a number between 1 and " + availablePlayers.size());
        }
        
        return availablePlayers.get(choice - 1);
    }

    @Override
    public Card selectCard(Player targetPlayer) {
        if (targetPlayer == null) {
            throw new IllegalArgumentException("Target player cannot be null");
        }

        List<Card> hand = targetPlayer.getHand();
        if (hand.isEmpty()) {
            throw new IllegalArgumentException("Target player has no cards");
        }

        System.out.println("\nTarget player's cards:");
        for (int i = 0; i < hand.size(); i++) {
            Card card = hand.get(i);
            System.out.println((i + 1) + ". " + card.getType());
        }
        System.out.print("Select a card (1-" + hand.size() + "): ");
        
        int choice = scanner.nextInt();
        if (choice < 1 || choice > hand.size()) {
            throw new IllegalArgumentException(
                "Invalid choice. Please enter a number between 1 and " + hand.size());
        }

        Card selectedCard = hand.get(choice - 1);
        if (!isFunctionalCard(selectedCard)) {
            throw new IllegalStateException("Selected card must be a functional card");
        }
        
        return selectedCard;
    }

    private boolean isFunctionalCard(Card card) {
        CardType type = card.getType();
        return type != CardType.CAT_CARD && type != CardType.EXPLODING_KITTEN;
    }
} 